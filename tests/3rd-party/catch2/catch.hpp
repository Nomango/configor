...
// https://gcc.gnu.org/onlinedocs/gcc-4.8.2/libstdc++/api/a01053_source.html line
// 2812-2813
...
namespace Catch
{
  ...
  bool
  matchTest(TestCase const & testCase, TestSpec const & testSpec, IConfig const & config);
...
// Applies the function macro `f` to each of the remaining parameters, inserts commas
// between the results, and passes userdata as the first parameter to each invocation,
...
#define INTERNAL_CATCH_TESTCASE_METHOD_NO_REGISTRATION(TestName, ClassName, ...)         \
  ...
  void TestName::test()
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION_2(TestName,                    \
                                                            TestFunc,                    \
                                                            Name,                        \
                                                            Tags,                        \
                                                            Signature,                   \
...
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_NO_REGISTRATION_2(TestNameClass,        \
                                                                   TestName,             \
                                                                   ClassName,            \
                                                                   Name,                 \
                                                                   Tags,                 \
...
#ifndef CATCH_CONFIG_TRADITIONAL_MSVC_PREPROCESSOR
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_SIG_NO_REGISTRATION(ClassName,          \
                                                                     Name,               \
                                                                     Tags,               \
                                                                     Signature,          \
                                                                     ...)                \
...
#pragma warning(disable : 4312) // Converting int to T* using reinterpret_cast (issue on
                                // x64 platform)
...
namespace Catch
{
  ...
  template <typename LhsT, typename RhsT> class BinaryExpr : public ITransientExpression
  {
   ...
   public:
    ...
    template <typename T>
    auto
    operator&&(T) const -> BinaryExpr<LhsT, RhsT const &> const
    {
    ...
    {                                                                                    \
      ...
      if (catchAssertionHandler.allowThrows())                                           \
        try                                                                              \
          ...
          {                                                                              \
            Catch::handleExceptionMatchExpr(catchAssertionHandler,                       \
                                            matcher,                                     \
                                            #matcher##_catch_sr);                        \
          }                                                                              \
      else                                                                               \
...
namespace Catch
{
  ...
  // !!! Be aware, returns refs into original string - make sure original string outlives
  // them
...
#pragma clang diagnostic ignored "-Wnon-virtual-dtor"
      ...
      // use of const for Object pointers is very uncommon and under ARC it causes some
      // kind of signature mismatch that breaks compilation
...
namespace Catch
{
  ...
  namespace Generators
  {
    ...
    // Tag type to signal that a generator sequence should convert arguments to a specific
    // type
    ...
    //       return type. Yeah.
    auto
    generate(StringRef generatorName,
             SourceLineInfo const & lineInfo,
    ...
    template <typename T> class RepeatGenerator : public IGenerator<T>
    {
     ...
     public:
      ...
      next() override
      {
        ...
        // If it happily moves, we are left in that state, otherwise it is time to start
...
#pragma optimize("", on)
    ...
    inline auto
    invoke_deoptimized(Fn && fn, Args &&... args) ->
...
namespace Catch
{
  namespace Benchmark
  {
    namespace Detail
    {
      ...
      /// effectively an implementation of std::function without any such optimizations;
...
namespace
{
  ...
  erf_inv(double x)
  {
    // Code accompanying the article "Approximating the erfinv function" in GPU Computing
    // Gems, Volume 2
    double w, p;
...
namespace Catch
{
  namespace Detail
  {
    ...
    Approx::equalityComparisonImpl(const double other) const
    {
      ...
      // Approx's value Thanks to Richard Harris for his help refining the scaled margin
      // value
    ...
    Approx::setEpsilon(double newEpsilon)
    {
      CATCH_ENFORCE(newEpsilon >= 0 && newEpsilon <= 1.0,
                    "Invalid Approx::epsilon: "
                      << newEpsilon << '.' << " Approx::epsilon has to be in [0, 1]");
...
namespace Catch
{
  ...
  void
  AssertionHandler::handleMessage(ResultWas::OfType resultType, StringRef const & message)
  {
...
namespace Catch
{
  ...
  // and infers the Equals matcher (so the header does not mention matchers)
  void
  handleExceptionMatchExpr(AssertionHandler & handler,
                           StringMatcher const & matcher,
                           StringRef const & matcherString)
...
namespace Catch
{
  namespace clara
  {
    namespace detail
    {
      ...
      // strings as a single string may encode an option + its argument if the : or = form
      // is used
...
namespace Catch
{
  ...
  void
  ExceptionTranslatorRegistry::registerTranslator(const IExceptionTranslator * translator)
  {
...
namespace Catch
{
  ...
  namespace Generators
  {
    ...
    auto
    acquireGeneratorTracker(StringRef generatorName, SourceLineInfo const & lineInfo)
      -> IGeneratorTracker &
    {
  ...
  namespace
  {
                        ...
                        private NonCopyable
    {
     ...
     public: // IMutableRegistryHub
      ...
      void
      registerTagAlias(std::string const & alias,
                       std::string const & tag,
...
namespace Catch
{
  ...
  IStreamingReporterPtr
  ReporterRegistry::create(std::string const & name, IConfigPtr const & config) const
  {
...
namespace Catch
{
  ...
  void
  RunContext::testGroupEnded(std::string const & testSpec,
                             Totals const & totals,
                             std::size_t groupIndex,
                             std::size_t groupsCount)
  {
  ...
  RunContext::assertionEnded(AssertionResult const & result)
  {
    ...
    // messages were made scoped and should be let to clear themselves out.
    static_cast<void>(
  ...
  void
  RunContext::runCurrentTest(std::string & redirectedCout, std::string & redirectedCerr)
  {
    auto const & testCaseInfo = m_activeTestCase->getTestCaseInfo();
    SectionInfo testCaseSection(testCaseInfo.lineInfo, testCaseInfo.name);
    ...
    SectionStats testCaseSectionStats(testCaseSection,
                                      assertions,
                                      duration,
                                      missingAssertions);
    m_reporter->sectionEnded(testCaseSectionStats);
...
namespace Catch
{
  ...
  StartupExceptionRegistry::add(std::exception_ptr const & exception) noexcept
  {
    ...
    CATCH_CATCH_ALL
    {
      // If we run out of memory during start-up there's really not a lot more we can do
      // about it
      std::terminate();
    }
  }
...
namespace Catch
{
                                                 ...
                                                 SourceLineInfo const & lineInfo)
  {
    ...
    CATCH_CATCH_ALL
    {
      // Do not throw when constructing global objects, instead register the exception to
      // be processed later
...
namespace Catch
{
  ...
  void
  TagAliasRegistry::add(std::string const & alias,
                        std::string const & tag,
                        SourceLineInfo const & lineInfo)
  {
...
namespace Catch
{
  ...
  TestCase
  makeTestCase(ITestInvoker * _testCase,
               std::string const & _className,
               NameAndTags const & nameAndTags,
               SourceLineInfo const & _lineInfo)
  {
...
namespace Catch
{
  ...
  std::vector<TestCase>
  filterTests(std::vector<TestCase> const & testCases,
              TestSpec const & testSpec,
              IConfig const & config)
  {
...
namespace Catch
{
  ...
  TestSpecParser::preprocessPattern()
  {
    ...
    for (std::size_t i = 0; i < m_escapeChars.size(); ++i)
      token =
        token.substr(0, m_escapeChars[i] - i) + token.substr(m_escapeChars[i] - i + 1);
    m_escapeChars.clear();
...
#include <cstddef>
  ...
  std::string
  StringMaker<std::nullptr_t>::convert(std::nullptr_t)
  {
    return "nullptr";
  }
    ...
    for (std::size_t idx = 0; idx < m_str.size(); ++idx)
      {
        ...
        switch (c)
          ...
          default:
            ...
            // encoding format First check that this bytes is a valid lead byte: This
            // means that it is not encoded as 1111 1XXX Or as 10XX XXXX
            if (c < 0xC0 || c >= 0xF8)
              {
                hexEscapeChar(os, c);
                break;
...
namespace Catch
{
  ...
  ConsoleReporter::ConsoleReporter(ReporterConfig const & config)
    ...
    , m_tablePrinter(
        new TablePrinter(config.stream(), [&config]() -> std::vector<ColumnInfo> {
  ...
  // subsequent lines
  void
  ConsoleReporter::printHeaderString(std::string const & _string, std::size_t indent)
  {
...
#define CATCH_REQUIRE_THAT(arg, matcher)                                                 \
  INTERNAL_CHECK_THAT("CATCH_REQUIRE_THAT",                                              \
                      matcher,                                                           \
                      Catch::ResultDisposition::Normal,                                  \
                      arg)
#endif // CATCH_CONFIG_DISABLE_MATCHERS
