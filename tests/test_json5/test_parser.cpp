...
TEST_CASE("test_json5_parser")
{
    ...
    SECTION("test_error_policy")
    {
        ...
        CHECK_THROWS_AS(json5::parse("\f", &strict_handler), configor_deserialization_error);

        error_handler_with<error_policy::ignore> ignore_handler{};
        CHECK_NOTHROW(json5::parse("\f", &ignore_handler));

        error_handler_with<error_policy::record> record_handler{};
        CHECK_NOTHROW(json5::parse("\f", &record_handler));
                              ...
                              { { "GlossEntry",
                                  {
                                      ...
                                      { "Abbrev", "ISO 8879:1986" },
                                      {
                                          ...
                                          { { "para", "A meta-markup language, used to create markup languages such "
                                                      "as DocBook." },
                                            { "GlossSeeAlso", json5::array({ "GML", "XML" }) } },
                                      },
                                      { "GlossSee", "markup" },
                                  } } } } } } } }
                };
