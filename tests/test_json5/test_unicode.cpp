...
#define TEST_FLOAT 1.25
        ...
        u16json5 u16j;
        CHECK_NOTHROW(u16j = u16json5::parse(U16(STR(TEST_INT))));
        CHECK(u16j.get<int64_t>() == TEST_INT);
        CHECK(u16j.dump() == U16(STR(TEST_INT)));

        CHECK_NOTHROW(u16j = u16json5::parse(U16(STR(TEST_FLOAT))));
        CHECK(u16j.get<double>() == TEST_FLOAT);
        CHECK(u16j.dump() == U16(STR(TEST_FLOAT)));

        u32json5 u32j;
        CHECK_NOTHROW(u32j = u32json5::parse(U32(STR(TEST_INT))));
        CHECK(u32j.get<int64_t>() == TEST_INT);
        CHECK(u32j.dump() == U32(STR(TEST_INT)));

        CHECK_NOTHROW(u32j = u32json5::parse(U32(STR(TEST_FLOAT))));
    ...
    {
        json5 j;
        CHECK_NOTHROW(j = json5::parse(ESCAPED_STR));
        CHECK(j.get<std::string>() == RAW_STR);

        wjson5 wj;
        CHECK_NOTHROW(wj = wjson5::parse(WIDE(ESCAPED_STR)));
        CHECK(wj.get<std::wstring>() == WIDE(RAW_STR));

        u16json5 u16j;
        CHECK_NOTHROW(u16j = u16json5::parse(U16(ESCAPED_STR)));
        CHECK(u16j.get<std::u16string>() == U16(RAW_STR));

        u32json5 u32j;
        CHECK_NOTHROW(u32j = u32json5::parse(U32(ESCAPED_STR)));
    ...
    SECTION("test_ignore_encoding")
    {
        {
            json5 j;
            CHECK_NOTHROW(j = json5::parse<encoding::ignore>(QUOTE_STR));
            CHECK(j.get<std::string>() == RAW_STR);
        }
        {
            json5 j = RAW_STR;
    ...
    SECTION("test_write_to_stream_w")
    {
        wjson5 j = {
            { WIDE("pi"), 3.141 },
            { WIDE("happy"), true },
